...
# 
#  python_script_file = "~/Python/mat2sif.0.2-hubs.py"
...
#    
#    getFunctionalProfile(gene.list = neighbours[[x]], universe = rownames(expr_data), filename = paste("Functional.profile.",x,".txt",sep=""), organism.db = "org.Hs.eg.db", organism = "human", golevels = F, kegg.organism = "hsa")
#    
#  })
#  
#
...
#  ##################### EXTRACT TARGET PATHWAYS ##########################
  ...
  target_pathways = as.character(read.delim("target_pathways.txt", header=F, stringsAsFactors=F)[,1])
...
for(p in names(path_tables)){
	...
	write.table(t(to_write), file = paste0("Target.pathway.",p,".enrichment.scores.in.neighbourhood.SNP.targets.50kb.txt"), quote=F, sep = "\t", col.names=NA)
