...
survit <- function(stime, event, expr, X=list(), split, smin=(length(stime)/10), gene.label="", data.label="Gene Expression", boxplot.ylab="Gene Expression", covariates=c(),plot=FALSE){
...
survproves <- function(id.sel, t1, t2, smin=(length(id.sel)/10), gene, ageopti=TRUE){
...
sfExport("sel")
sfExport("tar")
sfExport("clinical_info.cnv")
sfExport("clinical_info.expr")
sfExport("getstime")
sfExport("survit")
sfExport("cnv_data_combat_unique.sel")
sfExport("combat_data_unique.sel")
sfExport("survprobes")
sfExport("Surv")

sfSapply(tar, function(x)survproves(id.sel=sel, t1="CNV - Untreated GBM corrected - age factor",t2="Expression - Untreated GBM corrected - age factor", gene=x)) -> tar.full
...
expr.p_gene <- summary(x[["expr"]][["CA"]])[["coefficients"]]["xLow Risk","Pr(>|z|)"]
...
cnv.p_gene <- summary(x[["cnv"]][["CA"]])[["coefficients"]]["xLow Risk","Pr(>|z|)"]
