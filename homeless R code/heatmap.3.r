heatmap.3 <- function(x, trace="none", col=greenred(100), Colv=FALSE, dendrogram="row",mar=c(8,25),scale="none",Rowv=TRUE,...){

heatmap.2(x, trace=trace, col=col, Colv=Colv, dendrogram=dendrogram, distfun=function(x)as.dist(1-cor(t(x))), hclustfun=function(x)hclust(x, "average"), mar=mar, scale=scale,Rowv=Rowv,...)
}
